...
const GeneratePdf = ({
  closePreviewer,
  props: { roomId, roomNumber, buildingName },
}) => {
  ...
  const getRoomVisitors = async room_id => {
    ...
    await axios
      .post(
        'https://univtraze.herokuapp.com/api/rooms/searchUsersByRoomId',
        { room_id },
        {
          headers: headers,
        },
  ...
  const downloadPDF = () => {
    ...
    doc.html(document.querySelector('#dataContainer'), {
      callback: function (pdf) {
        pdf.save(
          `Room-${roomId}-${roomNumber}-${buildingName}-reportPDF(${moment().format(
            'YYYY-MM-DD',
          )}).pdf`,
        );
      },
  ...
  return (
    <div
      style={{
        ...
        height: '100vh',
        position: 'fixed',
        backgroundColor: 'rgba(54, 77, 57, .5)',
        alignItems: 'center',
