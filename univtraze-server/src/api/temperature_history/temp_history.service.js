...
module.exports = {
  getAllTempHistory: (callBack) => {
    pool.query(`SELECT * FROM temperature_history ORDER BY id DESC LIMIT 50;`, (error, results, fields) => {
      if (error) {
  ...
  getTempHistoryByUserId: (id, callBack) => {
    pool.query(
      `SELECT * FROM temperature_history WHERE temperature_history.user_id = ? ORDER BY id DESC LIMIT 50;`,
      [id],
