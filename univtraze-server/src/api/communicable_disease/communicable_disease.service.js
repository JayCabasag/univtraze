...
module.exports = {
  getAllCommunicableDiseaseReported: (callBack) => {
    pool.query(`SELECT * FROM reports`, (error, results, fields) => {
      if (error) {
  ...
  getAllCommunicableDisease: (callBack) => {
    pool.query(`SELECT DISTINCT disease_name FROM reports`, (error, results, fields) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
  ...
  getCommunicableDiseaseById: (data, callBack) => {
    pool.query(`SELECT * FROM reports WHERE id = ?`, [data.id], (error, results, fields) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
  ...
  deleteCommunicableDisease: (data, callBack) => {
    pool.query(`DELETE FROM reports WHERE id = ?`, [data.id], (error, results, fields) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
