...
module.exports = {
  ...
  getUniqueDiseases: (callBack) => {
    pool.query('SELECT disease_name, COUNT(*) AS total_cases, CAST(SUM(CASE WHEN status = true THEN 1 ELSE 0 END) AS INTEGER)  AS resolved_cases, CAST(SUM(CASE WHEN status = false THEN 1 ELSE 0 END) AS INTEGER) AS active_cases FROM disease_reports GROUP BY disease_name ORDER BY total_cases DESC;', [], (error, results, fields) => {
      if (error) {
        return callBack(error);
      }
      return callBack(null, results);
